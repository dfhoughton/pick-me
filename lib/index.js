...
exports.rando = exports.pickMeToo = exports.pickMe = void 0;
/**
 ...
 */
function pickMe(frequencies, rando) {
    return pickMeToo(frequencies)(rando !== null && rando !== void 0 ? rando : (() => Math.random()));
}
exports.pickMe = pickMe;
/**
 ...
 */
function pickMeToo(frequencies) {
    const simpleton = simplePicker(frequencies);
    if (simpleton)
        return simpleton;
    frequencies = sanitize(frequencies);
    // short-circuit edge cases
    if (frequencies.length === 0)
        throw new Error(`nothing to select in ${frequencies}`);
    if (frequencies.length === 1) {
        const [t] = frequencies[0];
        return (_rng) => () => t;
    }
    dupChecker(frequencies, (item) => item[0]);
    let total = 0, acc = 0;
    ...
    return (rng) => () => items[f(rng())];
}
exports.pickMeToo = pickMeToo;
...
function dupChecker(frequencies, toT) {
    for (let i = 0, lim = frequencies.length - 1; i < lim; i++) {
        const t = toT(frequencies[i]);
        for (let j = i + 1; j < frequencies.length; j++) {
            if (t === toT(frequencies[j]))
                throw new Error(`${t} appears more than once among frequencies`);
        }
    }
}
// if all frequencies are the same, we can make a simpler picker
function simplePicker(frequencies) {
    if (frequencies.length === 0)
        return;
    if (frequencies.every((s) => Array.isArray(s) && s.length === 2 && typeof s[1] === 'number')) {
        const castFrequencies = frequencies;
        const [, n1] = castFrequencies[0];
        if (n1 > 0 && castFrequencies.every(([, n2]) => n2 === n1)) {
            const items = castFrequencies.map(([t]) => t);
            dupChecker(items, (item) => item);
            return (rng) => () => items[Math.floor(items.length * rng())];
        }
    }
    else {
        const items = [...frequencies];
        if (items.length === 1) {
            const t = items[0];
            return (_rng) => () => t;
        }
...
function sanitize(frequencies) {
    return frequencies.map(([t, n]) => [t, Number(n)]).filter(([, n]) => Number.isFinite(n) && n > 0);
}
...
function thresholder(thresholds) {
...
function portion(thresholds, start, end) {
    const n = end - start;
    if (n === 1)
